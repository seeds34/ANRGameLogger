...
import org.seeds.anrgamelogger.application.database.buisnessobjects.LoggedGameFlat.LoggedGamesFlatColumns;
...
//            + "FOREIGN KEY("+GameNotesContract.GameNotesColumns.GAME_ID +") REFERENCES "+ Tables.LOGGED_GAME_OVERVIEWS +"("+BaseColumns._ID+"));";
    ...
    public void onUpgrade(SQLiteDatabase sqLiteDatabase, int oldVersion, int newVersion) {
        try{
            ...
            Log.e(LOG_TAG, "onUpgrade: "+e.getMessage() );
        }
    }
    ...
    public void importDatabase() throws IOException {
        String str = "/data/user/0/org.seeds.anrgamelogger/databases/" + DATABASE_NAME;
        importDatabase(str);
    }

    public void importDatabase(String databaseName) throws IOException {
        Log.d(LOG_TAG, "Starting Database Import");

        String fileIn = "/data/user/0/org.seeds.anrgamelogger/databases/ANRGAMELOGGER_BACKUP.db";
        String fileOut = context.getDatabasePath(DATABASE_NAME).toString();
        ;
        ...
        Log.d(LOG_TAG, "Ending Databse Import");
    }

    public void exportDaatabase() throws IOException {
        String str = "/data/user/0/org.seeds.anrgamelogger/databases/ANRGAMELOGGER_BACKUP.db";
        exportDaatabase(str);
    }

    public void exportDaatabase(String databaseName) throws IOException {

        Log.d(LOG_TAG, "Starting Database Export");
        ...
        String fileOut = databaseName;
        File f = new File(fileOut);
        f.createNewFile();
        FileOutputStream fOut = new FileOutputStream(f);

        Log.d(LOG_TAG, "Creating file in and as: " + fileOut);
        byte[] buffer = new byte[1024];
        int length;
        while ((length = fis.read(buffer)) > 0) {
            fOut.write(buffer, 0, length);
        ...
        Log.d(LOG_TAG, "Ending Database Export");
    }
}
